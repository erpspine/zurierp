...
@php
    ...
    $intro = match ($statusType) {
        'cancelled' => 'Your company subscription has been cancelled. Access to the tenant portal may stop immediately if there is no other active subscription.',
        'expired' => 'Your company subscription has expired. Tenant users will no longer be able to access the platform until the subscription is renewed.',
        default => $daysRemaining === 0
            ? 'Your company subscription expires today. Renew it now to avoid access interruption.'
            : 'Your company subscription is about to expire. Renew it before the end date to keep uninterrupted access.',
    };

    $accent = match ($statusType) {
        'cancelled' => '#8f2d1f',
        'expired' => '#7b241c',
        default => '#8a5c00',
    };
...
<div style="max-width: 680px; margin: 0 auto;">
    <div style="background: linear-gradient(135deg, #0f3a28 0%, {{ $accent }} 100%); border-radius: 18px 18px 0 0; padding: 24px 28px; color: #ffffff;">
        ...
        <p style="margin: 10px 0 0; font-size: 15px; line-height: 1.6; color: rgba(255,255,255,0.86);">{{ $intro }}</p>
    </div>
        ...
        <div style="border: 1px solid #e3ece7; border-radius: 16px; overflow: hidden; background: #fbfdfc; margin-bottom: 22px;">
            ...
            <div style="padding: 8px 18px 4px;">
                <div style="padding: 10px 0; border-bottom: 1px solid #ebf2ee;">
                    ...
                    <div style="font-size: 16px; font-weight: 700; color: #0d2f21;">{{ $subscription->company->name }}</div>
                </div>
                <div style="padding: 10px 0; border-bottom: 1px solid #ebf2ee;">
                    <div style="font-size: 11px; letter-spacing: 0.12em; text-transform: uppercase; color: #668073; margin-bottom: 4px;">Plan</div>
                    <div style="font-size: 15px; font-weight: 700; color: #0d2f21;">{{ $subscription->plan?->name ?? 'Custom Plan' }}</div>
                </div>
                <div style="padding: 10px 0; border-bottom: 1px solid #ebf2ee;">
                ...
                <div style="padding: 10px 0 14px;">
                    <div style="font-size: 11px; letter-spacing: 0.12em; text-transform: uppercase; color: #668073; margin-bottom: 4px;">End Date</div>
                    <div style="display: inline-block; padding: 10px 14px; border-radius: 10px; background: #eef6f1; border: 1px solid #d6e7dc; font-size: 15px; font-weight: 700; color: #0d2f21;">{{ optional($subscription->ends_at)->format('d/m/Y') }}</div>
                </div>
            </div>
        </div>
